...
1. 以下のコマンドを実行し、初期コンテキストを標準出力として取得する:

   ```bash
   bash scripts/collect_structure.sh <対象パス> # 必要に応じて最大階層を指定
   ```

   **取得される情報**:
   - ディレクトリツリーとファイル統計
   - **依存関係マップ (import/export の結合関係)**
   - **主要エクスポート・関数シグネチャ (クラス定義、引数、戻り値、DocString の一覧)**
...
エントリーポイントを「ファイル閲覧ツール」等で読み、import されているモジュールを**最大2階層**辿る。
...
`collect_structure.sh` のファイルサイズ Top15-20 に対して「ファイルアウトライン抽出ツール」（または通常の内容確認機能）を実行する。
...
> 構造を生成した後、**絶対に Phase 3 (ページ作成) に進んではいけません。**
> 必ずユーザーに対して「この構造でページ生成を開始してよいか」の承認を求めてください。ユーザーから明示的な許可（「OK」「進めて」など）を得るまでは、絶対に次のステップに進まないこと。
>
> **【提示フォーマット: JSON禁止・Markdown表形式で提示する】**
> Wiki構造をユーザーに提示する際は、JSONをそのまま貼り付けるのではなく、**人間が読みやすいMarkdownの箇条書き・表形式**で提示すること。以下のフォーマットを厳守する：
>
> ```
...
> | 1.3 | Configuration System | medium | `src/config/`, `*.config.ts` |
>
> ### Section 2: Data Layer（データ層）
> | # | ページタイトル | importance | 主な参照ファイル |
> |---|---|---|---|
> | 2.1 | Data Models | medium | `src/models/*.ts` |
> | 2.2 | Repository Pattern | medium | `src/repositories/*.ts` |
>
> （以下、全セクション続く）
>
...
1. ページの `filePaths` に記載された**全ファイル**を「ファイル閲覧ツール」等で読む
2. **ファイル読解直後に、参照した行範囲を必ずメモする**（例: `config.ts: L1-L80 読了`）
3. 主要なクラス・関数を「内容確認ツール」等で詳細確認する
4. **「コード検索ツール」等でクラス名・関数名の呼び出し元・参照先を必ず追跡する**（省略禁止）
5. **Deep Research (詳細調査ループ)**: `importance: high` のページの場合、ファイルを読むだけで終わらせず、以下のループを回して技術的深度を確保する。
   - **仮説構築**: モジュールの中心的な役割やデータフローの仮説を立てる
   - **深層追跡**: 仮説に基づき、さらに2層以上深い依存ファイルや共通ユーティリティを特定・確認する
   - **結論抽出**: 分かった設計意図や複雑なビジネスロジックを言語化する
6. **分析メモを以下の形式で整理する**：
...
> **スニペット候補が 5 個未満の場合は、追加のファイルを「ファイル閲覧ツール」等で読む。**
...
生成したページを「ファイル書き込みツール」で即座にファイルに書き出す。**次のページに進む前に必ず書き出す。**

#### Step 3d: 品質検証（バリデーションループ）

> [!CAUTION]
> **【超重要: ページ単位での順次検証の絶対厳守】**
> バリデーションを後回しにして、複数のページや全ページを一気に生成するツール呼び出し（バッチ処理）は**絶対に行ってはいけません**。
> 「1つのページを生成出力する」→「必ずそのファイルに対して `validate_page.py` を実行する」→「結果（Grade）を読み取る」→「必要なら修正する」というプロセスを完全に終えるまで、**絶対に次のページの生成に着手してはなりません。**

生成した各ページに対して、以下のバリデーションスクリプトを**都度必ず実行**する：

```bash
python3 scripts/validate_page.py <生成したファイル.md> --importance <high|medium|low>
```

1. **Grade B 以上 (75%+)**: 合格。ここで初めて次のページに進む。
2. **Grade C (60-74%)**: 指摘された `❌` または `⚠️` の項目を修正し、再検証を実行。最大2回まで修正ループを回す。
...
4. **コードスニペット（最重要）**: **実際のソースコードからの引用のみ許可**（疑似コード禁止）
   - ファイル閲覧時に読んだソースコードの中から、**そのページのトピックを象徴する部分を抜粋**する
...
| ソース参照がない / 行番号がない | Phase 3a で **ファイル閲覧直後に行番号をメモ**し、3b で Sources 行に転記 |
...
| コードが疑似コード | 実際のソースからファイル閲覧ツール等で抜粋する |
