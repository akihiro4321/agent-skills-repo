...
| **追加ディレクトリ** | （省略可）周辺サービス等、Wiki に含めたい他サービスのルートパス。`outline.json` の `additionalDirs` に設定し、`filePaths` で各ページが参照するファイルを明示する | `/path/to/user-service`, `/path/to/payment-service` |

---

## Phase 0.5: 実行環境のセットアップ（必須）

`deepwiki` スキルは高度なコード解析のために `tree-sitter` 等の外部パッケージを必要とします。**システム環境を汚染しないよう、必ず以下の手順でスキルのディレクトリに仮想環境を構築してください。** アクティベート（`source`）は不要です。以降のスクリプト実行はすべて `.venv/bin/python` の絶対パスで呼び出します。

```bash
# このスキルが置かれているディレクトリの絶対パスを確定する
DEEPWIKI_DIR="/path/to/skills/deepwiki"  # ← 実際のパスに置き換えること

if [ ! -d "$DEEPWIKI_DIR/.venv" ]; then
  python3 -m venv "$DEEPWIKI_DIR/.venv"
  "$DEEPWIKI_DIR/.venv/bin/pip" install --upgrade pip
  "$DEEPWIKI_DIR/.venv/bin/pip" install tree-sitter==0.21.3 tree-sitter-javascript==0.21.4 tree-sitter-typescript==0.21.2 tree-sitter-java==0.21.0
fi

# 以降のスクリプト呼び出しで使う変数（このセッション中は保持すること）
DEEPWIKI_PYTHON="$DEEPWIKI_DIR/.venv/bin/python"
```

> [!IMPORTANT]
> `DEEPWIKI_PYTHON` 変数を定義した後、以降の Phase 1 〜 4 でPythonスクリプトを実行する際は、必ずこの変数を使用してください。`python3` コマンドは使用しないこと。
...
> [!IMPORTANT]
> **周辺サービスのファイルを Wiki に含める場合、`filePaths` に各サービスのファイルの絶対パスを明示すること。** `additionalDirs` はGeminiのアクセス許可範囲を広げるだけであり、`filePaths` に含まれていないファイルはGeminiが能動的に参照しない。周辺サービスのどのファイルを参照するかは、Phase 1 の分析結果をもとにメインエージェント（あなた）が判断して `filePaths` に列挙する。
...
| `additionalDirs` | string[] | （省略可）周辺サービス等の追加ディレクトリパス（絶対パス）。Gemini CLIの `--include-directories` に追加され、アクセス許可範囲を広げる。**どのファイルを読むかは `filePaths` で明示すること**（このフィールドのみ設定してもGeminiは自律的に読みに行かない） |
...
```bash
"$DEEPWIKI_PYTHON" "$DEEPWIKI_DIR/scripts/generate_pages.py" $OUTPUT_DIR/outline.json
```
...
```bash
"$DEEPWIKI_PYTHON" "$DEEPWIKI_DIR/scripts/fix_mermaid.py" $OUTPUT_DIR/outline.json
```
...
```bash
# リモートURLを自動取得できる場合
"$DEEPWIKI_PYTHON" "$DEEPWIKI_DIR/scripts/fix_sources.py" $OUTPUT_DIR/outline.json --link-style github

# 取得できない場合（ユーザーから URL を聞いた後）
"$DEEPWIKI_PYTHON" "$DEEPWIKI_DIR/scripts/fix_sources.py" $OUTPUT_DIR/outline.json --link-style github --remote-url https://github.com/org/repo
```

**② vscode:// URL を選んだ場合**

```bash
"$DEEPWIKI_PYTHON" "$DEEPWIKI_DIR/scripts/fix_sources.py" $OUTPUT_DIR/outline.json --link-style vscode
```
...
1. **インデックスページ (`index.md`) の作成**:
   ...
   ```bash
   "$DEEPWIKI_PYTHON" "$DEEPWIKI_DIR/scripts/create_index.py" $OUTPUT_DIR/outline.json
   ```
2. 全体バリデーションの実行（必要に応じて）:
   ```bash
   "$DEEPWIKI_PYTHON" "$DEEPWIKI_DIR/scripts/validate_page.py" $OUTPUT_DIR
   ```

## Phase 5: 完了報告
生成された Wiki の出力先パスと、主要なページのハイライトをユーザーに報告する。
