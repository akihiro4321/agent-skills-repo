...
```typescript
// path/to/file.ts:L45-L60
(実際の重要コードを抜粋)
```

**Sources:** [file.ts:L45-L60](file:///path/to/file.ts#L45-L60)
...
- **全般的な構文エラー防止ルール:**
  - ノードラベルに `()`、`[]`、`{}` や記号類などの特殊文字を含む場合は必ず `["ラベル"]` でクォートすること。
  - **特に `D[Data Pipeline (api/data_pipeline.py)]` のように `[]` の内側に `()` が含まれるとパースエラーで図が崩壊します。必ず `D["Data Pipeline (api/data_pipeline.py)"]` とダブルクォートで囲むか、そもそも括弧を使わないようにしてください。**
  - **同様に `cmd_start_sh(CMD ["/app/start.sh"])` のように `()` の内側に `[]` 等が含まれるケースでもパースエラーになります。必ずクォートするか、`cmd_start_sh("CMD /app/start.sh")` または括弧なしにするなど工夫してください。**
  - HTMLタグは使用不可。
